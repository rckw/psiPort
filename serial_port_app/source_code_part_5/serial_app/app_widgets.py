...
class SerialMenuBar(QMenuBar):
    def __init__(self, parent=None):
        ...
        # Define the path to the 'commands' folder (we should replace to somewhere else. We just leave it here for now.)
        self.command_folder = Path.cwd() / 'commands'

        self.open_command_menu = command_menu.addMenu("Recent commands")
    ...
    def _create_actions(self):
        ...
        self.open_command_menu.aboutToShow.connect(self.populate_files)

    def populate_files(self):
        self.open_command_menu.clear()
        actions = []
        for path in self.command_folder.rglob("*.txt"):
            action = QAction(path.stem, self)
            action.triggered.connect(partial(self.send_command_file, path))
            actions.append(action)
        self.open_command_menu.addActions(actions)

    @classmethod
    def send_command_file(cls, file_name: Path):
        with file_name.open(mode='r', encoding='utf-8') as read_file:
            for line in read_file:
                line = line.strip()
                print(line)  # Later we will send the file content to the serial port
...
class SerialBox(QGroupBox):
    def __init__(self, parent=None):
        ...
        self.command_line.addItem("help")
        self.command_line.setCurrentIndex(len(self.command_line))
        ...
        self.command_line.currentIndexChanged.connect(self.send_command)

    def send_command(self):
        print(self.command_line.currentText())
        self.command_line.setCurrentIndex(len(self.command_line))
